...
This command sends a message to the `hello_world_backend` canister that has been deployed on the playground network (indicated by the `--network playground` flag), and calls the method `greet`, then passes the text `'("everyone")'` to the method. 
...
```sh
dfx canister --network playground call hello_world_backend greet '("developers")'
```
